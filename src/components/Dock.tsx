...
const Dock = () => {
    ...
    return (
        <div className={`px-5 lg:px-2 py-2 mt-3 md:mt-0 space-x-2 lg:space-x-0 lg:py-8 rounded-2xl md:rounded-3xl border ${theme=='dark'?"border-zinc-700/50":"border-zinc-400"} w-auto  lg:w-20 space-y-0 lg:space-y-2 inline-flex flex-row  lg:flex-col items-center`}>
            {icons.map((icon, index) => (
                <div
                    key={index}
                    className={`inline-block p-2 ${theme=='dark'?'icon':'dicon'} rounded-md`}
                >
